...
/// <summary>
/// Collects semantic edges in one traversal per source document.
/// </summary>
public sealed class CodeRelationshipIndexer
{
    private readonly SymbolIdentityService _identity;

    public CodeRelationshipIndexer(SymbolIdentityService identity)
    {
        _identity = identity;
    }
    ...
    public async Task<CodeRelationshipGraph> IndexAsync(IndexedProjectContext context, SymbolIndex symbols, CancellationToken cancellationToken = default)
    {
        ...
        foreach (Project project in context.Solution.Projects)
        {
            ...
            foreach (Document document in project.Documents)
            {
                ...
                {
                    if (model.GetDeclaredSymbol(declaration, cancellationToken) is not INamedTypeSymbol source)
                    {
                        continue;
                    }
                ...
                foreach (SyntaxNode node in root.DescendantNodes())
                {
                    ...
                    if (owner is null || !internalIds.Contains(_identity.GetId(owner)))
                    {
                        continue;
                    }
                    ...
                    {
                        Add(owner, called, CodeRelationshipType.Calls, node, path, result, seen, internalIds, true);
                    }
    ...
    private static IEnumerable<INamedTypeSymbol> ExpandTypes(ITypeSymbol? type)
    {
        ...
        {
            foreach (INamedTypeSymbol item in ExpandTypes(array.ElementType))
            {
                yield return item;
            }
        ...
        {
            foreach (INamedTypeSymbol item in ExpandTypes(pointer.PointedAtType))
            {
                yield return item;
            }
        ...
        {
            foreach (INamedTypeSymbol nested in ExpandTypes(argument))
            {
                yield return nested;
            }
        }
    ...
    {
        string sourceId = _identity.GetId(source);
        string targetId = _identity.GetId(target);
        ...
        if (!seen.Add(key) || sourceId == targetId && type == CodeRelationshipType.UsesType)
        {
            return;
        }
